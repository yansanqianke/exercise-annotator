...
- **手动输入**：选择学科 → 输入题目 → 保存到题库
- **文档批量提取**：上传 PDF/Word/PPT/TXT → LLM 自动识别题目并入库

### 题目管理 `/questions`

题库列表 + 标注 + 批量操作：
1. 左侧表格：按学科/题型筛选，多选题目
2. 右侧面板：题目详情 + 标注结果
3. 点击"标注"→ LLM 流式推理 → 输出题型/难度/知识点
4. 选中多题 → 批量标注
5. 手动修正 KP、删除题目

### 参考资料 `/documents`

上传 PDF / Word / PPT / TXT 文档 → 索引分块写入 Chroma，供标注管道语义检索。
